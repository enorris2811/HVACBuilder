...
},'sequence':"<strong>RunConditions</strong> <p>The Unit will supply conditioned air to its designated terminal units and operate as a variable air volume unit.  The unit will run according to the folling user selectable modes:</p> <ul class='list-group'> <li class='list-group-item'> Off </li> <li class='list-group-item'> Continuously </li> <li class='list-group-item'> via BAS Command </li> <li class='list-group-item'> as Locally Scheduled </li> <li class='list-group-item'> via Linked Scheduled </li> </ul> <strong>Off Mode:</strong> <p>The unit will not run and all dampers, valves and fans will remain in their off position</p> <strong>Continuously</strong> <p>The unit will run continuously in the occupied mode.</p> <strong>via BAS Command</strong> <p>The unit will be enabled and disabled via a Bacnet Binary command, either from a remote source, or locally through the software interface. once enabled, the unit will run continuously in the occupied mode.</p> <strong>as Locally Scheduled</strong> <p>The air handling unit will operate as a variable volume unit, but one having setpoints that represent those of a single zone unit.  The unit will then run according to a user definable time schedule in the following modes:</p> <ul class='list-group'>Occupied Mode: The unit will maintain: <li class='list-group-item'> A <input id='spaceCoolingSetpointOcc' type='number'> degrees F space cooling setpoint </li> <li class='list-group-item'> A <input id='spaceHeatingSetpointOc' type='number'> degrees F space heating setpoint </li> </ul> <ul class='list-group'>Unoccupied Mode: The unit will maintain: <li class='list-group-item'> A <input id='spaceCoolingSetpointUnocc' type='number'> degrees F space cooling setpoint </li> <li class='list-group-item'> A <input id='spaceHeatingSetpointUnocc' type='number'> degrees F space heating setpoint </li> </ul> <strong>Unit Optimal Start</strong> <p>The unit will use an optimal start algorithm for morning start-up.  This alogorithm will minimize the unoccupied warm up or cool down period while still achieving comfort conditions by the start of scheduled occupied period.</p> <p>Alarms will be provided as follows:</p> <ul class='list-group'> <li class='list-group-item'> High Space Temp: If the zone temperature is greater than the cooling setpoint by a user definable amount. </li> <li class='list-group-item'> Low Space Temp:  if the zone temperature is less than the heating setpoint by a user definable amount. </li> </ul> <strong>via linked Schedule</strong> <p>The air handling unit will exchange information with the terminal units as follows:</p> <ul class='list-group'> <li class='list-group-item'> Calculated information sent from the air handling unit to the terminal units: <ul class='list-group'> <li class='list-group-item'> Current Mode of operation </li> <li class='list-group-item'> current static pressure </li> <li class='list-group-item'> current supply air temperature </li> <li class='list-group-item'> current outside air temperature </li> </ul> </li> <li class='list-group-item'> Calculated information representative of all terminal units sent from the terminal units to the air handling unit: <ul class='list-group'> <li class='list-group-item'> Space Temperature </li> <li class='list-group-item'> Setpoints, both occupied and unoccupied </li> <li class='list-group-item'> maximum damper position </li> <li class='list-group-item'> indoor air quality (if present) </li> <li class='list-group-item'> space relative humidity (if present) </li> </ul> </li> </ul> <strong>Power-up Start-up Delay</strong> <p>There will be a 60 second delay in enabling the equipment on intial start up or after a power loss.</p> <strong>Demand limiting - Setpoint Optimization</strong> <p>To lower power consumption, the cooling and heating setpoints will automatically relax (offset up when cooling and down when heating) when the facility power consumption exceeds definable thresholds.  the amount of relaxation will be user configurable.  The setpoints will automatically return to their previous settings when the facility power consumption drops below the thresholds.</p>" },
...
var safties = {
    'freezeProtection': {
        ...
        'sequence': '<strong>Freeze Protection</strong>:<br>The unit will shut down and generate an alarm upon receiving a freezestat status.<br><br>'
    },
    'returnAirSmokeDetector': {
        'isUsed': false,
        'points': { 0: 'returnAirSmokeDetector' },
        'sequence': '<strong>Return Air Smoke Detector</strong>:<br>The unit will shut down and generate an alarm upon recieving a signal that smoke is present in the Return Air.<br><br>'
    },
    'supplyAirSmokeDetector': {
        'isUsed': false,
        'points': { 0: 'supplyAirSmokeDetector' },
        'sequence': '<strong>Supply Air Smoke Detector</strong>:<br>The unit will shut down and generate an alarm upon receiving a signal that smoke is present in Supply Air.<br><br>'
    }
};
...
var smokeControl = {
    ...
    'purge': {
        'isUsed': false, 'points': {
            ...
            1: 'smokeRelayK1',
            2: 'smokeRelayK2'
        }, 'sequence': '<strong>Smoke Purge</strong>:<br>K1 Open(off), K2 Open(off)<br><br> Upon receiving a smoke purge signal, the unit will:<br><br><ul>Start the supply fan</ul><ul>And start the return fan</ul><ul>AND open the outdoor and exhaust air dampers</ul><ul>AND close the return damper</ul><br><br>Alarm will be provided as follows:<br><ul>Smoke Purge Alarm: Unit smoke purge input is on and unit is in area smoke purge mode.<br><br>'
...
var outdoorAirTemp = {
    'physicalInput': {
        ...
        'sequence': '<strong>Outdoor Air Conditions</strong>:<br>The controller will monitor outdoor air temperatur on a continual basis. These values will be made available to the system at all times.<br><br>'
    },
    'networkInput': {
        'isUsed': false,
        'points': {0:'outdoorAirTemperature'  },
...
var exhaustFan = {
        ...
        'buildingPressurization': {
            ...
            'sequence': '<strong>Exhuast Fan</strong>:<br>The Exhaust Fan will run whenever the supply fan runs.<br><br>Alarms will be provided as follows:<br><ul>Exhaust Fan Failure: Commanded on but the status is off</ul><br><ul>Exhaust Fan in Hand: Commanded off, but status is on.</ul><br><ul>Exhaust Fan Runtime Exceeded: Status runtime exceeds a user definable limit.</ul><br><br><strong>Building Static Pressure Control:</strong><br>The controller will measure building static pressue and modulate the exhaust fan VFD speed to maintain a building static pressure setpoint of <input type=\'number\'> in H2O.  The exhause fan VFD speed PID will be upper limited by keeping the Exhaust Fan Inlet of P1 differential pressure sensor more megative (fixed 0.05 in H20) than Supply Fan side of Mixed Air Damper P1 differential pressure sensor.<br><br>Alarm will be provided as follows:<br><ul><li>High building Static Pressure: If the building air static pressure is <input type=\'number\'> percent greater than the setpoint </li><li>Low Building Static Pressure: If the building air static pressure is <input type=\'number\'> less than the setpoint</li></ul><br><br>'
        }
    };
    ...
    var preHeating = {
        ...
        'electricStaging': {
            'isUsed': false, 'points': {
                ...
                'stages': null,
                'sequence': '<strong>Electric Preheating Stages - AP to Stage Sequencer</strong>:<br>The controller will measure the preheat discharge air temperature and stage the preheating through an external stage sequencer to maintain its setpoint <input type=\'text\' id=\'preheatTempSetpoint\' default=\'50\'> degrees F.  The preheating will be enabled whenever the supply fan status is on.<br><br>The preheating stages will run for freeze protection whenever:<br><br><ul>Preheat air temperature drops from <input type=\'text\' id=\'preheatDeadBandHigh\' default=\'40\'> degrees F to <input type=\'text\' id=\'preheatDeadbandLow\' default=\'35\'> degrees F.</ul><br><ul>AND the supply fan status is on.</ul><br><br>'
            }
        },
        'gasStaging': {
            'isUsed': false, 'points': {
        ...
        'SCRElectric': {
            ...
            }, 'sequence': '<strong>SCR Electric Heat</strong>:<br>The controller will measure the preheat discharge air temperature and modulate the output to the SCR control interface to maintain its setpoint <input type=\'text\' id=\'preheatTempSetpoint\' default=\'50\'> degrees F.  The preheating will be enabled whenever the supply fan status is on.<br><br>The preheating stages will run for freeze protection whenever:<br><br><ul>Mixed air temperature drops from <input type=\'text\' id=\'preheatDeadBandHigh\' default=\'40\'> degrees F to <input type=\'text\' id=\'preheatDeadbandLow\' default=\'35\'> degrees F.</ul><br><ul>AND the supply fan status is on.</ul><br><br>'
        },
        'hotWaterModulating': {
            'isUsed': false, 'points': {
                0: 'preheatDischargeAirTemperature',
                ...
                3: 'highPreheatDischargeAirTemperature',
                4: 'lowPreheatDischargeAirTemperature'
            }, 'sequence': '<strong>Preheating Coil Valve</strong>:<br>The controller will measure the preheat discharge air temperature and modulate the preheating coil valve to maintain its setpoint <input type=\'text\' id=\'preheatTempSetpoint\' default=\'50\'> degrees F.  The preheating stages will run for freeze protection whenever preheat air temperature drops from <input type=\'text\' id=\'preheatDeadBandHigh\' default=\'40\'> degrees F to <input type=\'text\' id=\'preheatDeadbandLow\' default=\'35\'> degrees F or if the freezestat is on and present.<br><br> '
        },
        'hotWaterFaceBypass': {
            'isUsed': false, 'points': {
                0: 'preheatDischargeAirTemperature',
            ...
            }, 'sequence': '<strong>Preheating Integral Face and Bypass Coil</strong>:<br>The controller will measure the preheat discharge air temperature and modulate the preheating integral bypass damper and open the coil valve to maintain its setpoint <input type=\'text\' id=\'preheatTempSetpoint\' default=\'50\'> degrees F.  The preheating coil steam valve will open for freeze protection whenever preheat air temperature drops from <input type=\'text\' id=\'preheatDeadBandHigh\' default=\'40\'> degrees F to <input type=\'text\' id=\'preheatDeadbandLow\' default=\'35\'> degrees F or if the freezestat is on and present.<br><br> '
        },
        'steamModulating': {
            'isUsed': false, 'points': {
    ...
    var heating = {
        ...
        'electricStaging': {
            'isUsed': false, 'points': {
                0: 'heatingStage',
                'stages': null
            }, 'sequence': '<strong>Electric Heating Stages</strong>:<br>The controller will measure the supply air temperature and stage the heating to maintain its heating setpoint.  to prevent short cycling there will be a user definable delay between stages, and each stage will have a user definable minimum runtime.<br><br>The heating will be enabled whenever:<br><br><ul>Outside aire temperature is less than <input type=\'text\' id=\'minOATSetpoin\' default=\'65\'> degrees F.</ul><br><ul>AND the supply air temperature is below heating setpoint.</ul><br><ul>AND the cooling (if present) is not active.</ul><br><ul>AND the supply fan status is on.</ul><br><br>'
        },
        'gasStaging': {
            'isUsed': false, 'points': {
                'stages': null
        ...
        'SCRElectric': {
            'isUsed': false, 'points': {
                0: 'SCRElectric'
            }, 'sequence': '<strong>SCR Electric Heat</strong>:<br>The controller will compare the space temperature to the space temperature setpoint, and compute a target Supply Air Temperature setpoint in order to satisfy the required Space Temperature conditions.The output to the SCR control interface will be modulated to main the target Supply Air setpoint. Th supply air temperature setpoint will be reset as needed to satisfy Space conditions.<br><br>The heating will be enabled whenever:<br><br><ul>Outside aire temperature is less than <input type=\'text\' id=\'minOATSetpoin\' default=\'65\'> degrees F.</ul><br><ul>AND the supply air temperature is below heating setpoint.</ul><br><ul>AND the cooling (if present) is not active.</ul><br><ul>AND the supply fan status is on.</ul><br><br>'
        },
        'hotWaterModulating': {
            'isUsed': false, 'points': {
                0: 'heatingValve'
            }, 'sequence': '<strong>Heating Coil Valve</strong>:<br>The controller will measure the supply air temperature and modulate the heating coil valve to maintain its setpoint.  The supply air temperature setpoint will be reset upwards as the space temperature drops below its heating setpoint.<br><br>The heating will be enabled whenever:<br><br><ul>Outside air temperature is less than <input type=\'text\' id=\'minOATSetpoint\' default=\'65\'> degress F.</ul><br><ul>AND the space temperature is below heating setpoint.</ul><br><ul>AND the supply fan status is on.</ul><br><ul>AND the cooling (if present) is not active.</ul><br><br>The heating coil valve will open whenever the freezestat (if present) is on.<br><br>'
        },
    ...
    var cooling = {
        ...
        'dXCooling': {
            'isUsed': false, 'points': {
                ...
                'stages': null
            }, 'sequence': '<strong>Cooling Stages:</strong><br>The controller will measure the space temperature and stage the cooling to maintain its cooling setpoint. To prevent short cycling, there will be a user definable delay between stages, and each stage will have a user definable minimum runtime.<br><br>The cooling will be enabled whenever:<br><br><ul>Outside air temperature is greater than <input type=\'text\' id=\'coolingSetpoint\' placeholder\'60\'> degree F</ul><br><ul>AND the economizer (if present)is disabled or fully open</ul><br><ul>AND the space temperature is above cooling setpoint</ul><br><ul>AND the supply fan status is on</ul><br><ul>AND the heating (if present) is not active</ul><br><br>'
        },
        'chilledWaterModulating': {
            'isUsed': false, 'points': {
                0: 'coolingVavle'
            }, 'sequence': '<strong>Cooling Coil Valve:</strong><br>The controller will measure the supply air temperature and modulate the cooling coil valve to maintain its cooling setpoint. The supply air temperature setoint will be reset downards as the space temperature rises above its cooling setpoint.<br><br>The cooling will be enabled whenever:<br><br><ul>Outside air temperature is greater than <input type=\'text\' id=\'coolingSetpoint\' placeholder\'60\'> degree F</ul><br><ul>AND the economizer (if present)is disabled or fully open</ul><br><ul>AND the space temperature is above cooling setpoint</ul><br><ul>AND the supply fan status is on</ul><br><ul>AND the heating (if present) is not active</ul><br><br>The cooling coil valve will open to <input type=\'text\' id=\'minCoolingCoilAnalog\' placeholder=\'50\'> percent whenever the freezestat (if present) is on<br><br>'
        },
        ...
        'chilledWaterExternalFaceBypass': {
            'isUsed': false, 'points': {
                0: 'coolingValve'
            }, 'sequence': '<strong>Cooling Coil Valve:</strong><br>The controller will measure the space temperature and open the cooling coil valve in sequence with the face and bypass dampers to maintain its cooling setpoint.<br><br>The cooling will be enabled whenever:<br><br><ul>Outside air temperature is greater than <input type=\'text\' id=\'coolingSetpoint\' placeholder\'60\'> degree F</ul><br><ul>AND the economizer (if present)is disabled or fully open</ul><br><ul>AND the space temperature is above cooling setpoint</ul><br><ul>AND the supply fan status is on</ul><br><ul>AND the heating (if present) is not active</ul><br><br>The cooling coil valve will open whenever the freezestat (if present) is on<br><br>'
        },
    ...
    var minOutsideAir = {
        ...
        'OADamperFlowStation': {
            ...
            'sequence': '<strong>Minimum Outside Air Ventilation:</strong><br>When in the occupied mode, the controller will measure the incoming outside airflow and compute an economizer position that will maintain the proper minimum outside air ventilation setpoint of <input type=\'number\'>. This will be compared to the current economizer position, and the larger of the two values will be sent to the economizer actuator.<br><br>'
        }
